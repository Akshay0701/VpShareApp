...
public class adminLoginFrag extends Fragment {
    ...
    private void LoginAdmin(String username, final String email, final String password) {
        ...
        //actual password and email store in database
        if (email.equals("vpshare123@gmail.com") && password.equals("VpShare@125")) {
            auth.signInWithEmailAndPassword(email, password)
                    .addOnCompleteListener(getActivity(), new OnCompleteListener<AuthResult>() {
                        @Override
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            if (task.isSuccessful()) {
                                //first here we going to check whether that email and password is already store or not if nott then do registration
                ...
                public void onFailure(@NonNull Exception e) {
                    progressDialog.dismiss();
                    Toast.makeText(getActivity(), "creating new email " + e.getMessage(), Toast.LENGTH_SHORT).show();
                    //here we are doing that registertion of admin email and password
